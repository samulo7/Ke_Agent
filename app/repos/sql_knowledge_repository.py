...
def bootstrap_sqlite_schema(
    ...
    *,
    ...
    """Create A-10 split tables for SQLite-based local verification."""
...
class SQLKnowledgeRepository(KnowledgeRepository):
    """A-10 SQL-backed repository with JOIN-based permission filtering."""
        ...
        *,
        ...
        WHERE d.status = ?
          ...
          AND (
              ...
              OR (
                  ? <> ''
                  AND d.permission_scope = 'department'
    ...
    def _row_to_entry(row: Any) -> KnowledgeEntry:
        source_type = str(row[1]).strip().lower()
        normalized_source_type = "faq" if source_type == "faq" else "document"
