...
FIXED_QUOTE_TARGET_KEYWORDS = ("型号", "配件", "组件", "定影器", "套件", "耗材")
FIXED_QUOTE_DISCOUNT_KEYWORDS = ("折扣", "优惠", "打折", "几折", "9折", "8折", "7折")
FIXED_QUOTE_INQUIRY_KEYWORDS = ("核价", "询价", "成本核算", "成本怎么算", "费用怎么算", "预算价")
...
class IntentClassifier:
    ...
    def classify(self, text: str) -> IntentClassification:
        ...
        if self._is_fixed_quote(question):
    ...
    def _is_fixed_quote(self, question: str) -> bool:
        if self._contains_any(question, FIXED_QUOTE_KEYWORDS):
            return True
        has_target = self._contains_any(question, FIXED_QUOTE_TARGET_KEYWORDS)
        has_discount_signal = self._contains_any(question, FIXED_QUOTE_DISCOUNT_KEYWORDS)
        if has_target and has_discount_signal:
            return True
        has_quote_inquiry = self._contains_any(question, FIXED_QUOTE_INQUIRY_KEYWORDS)
        return has_target and has_quote_inquiry
