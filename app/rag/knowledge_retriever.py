...
class KnowledgeRetriever:
    """A-08 deterministic retriever with document-first ranking."""
    ...
    @staticmethod
    def _score(entry: KnowledgeEntry, question: str) -> int:
        keyword_hits = sum(1 for keyword in entry.keywords if keyword in question)
        title_hit = 1 if _normalize(entry.title) in question else 0
        summary_hit = 1 if _normalize(entry.summary) in question else 0
