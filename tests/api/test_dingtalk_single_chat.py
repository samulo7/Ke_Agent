...
class DingTalkSingleChatApiTests(unittest.TestCase):
    ...
    def test_reimbursement_travel_workflow_happy_path_with_callback_submit(self) -> None:
        ...
        self.assertEqual("reimbursement_travel_recognition_confirmation", third.json()["reason"])
        self.assertEqual("interactive_card", third.json()["reply"]["channel"])

        confirmed_recognition = client.post(
            "/dingtalk/stream/events",
        ...
        self.assertEqual(200, confirmed_recognition.status_code)
        self.assertEqual("reimbursement_travel_collecting_company", confirmed_recognition.json()["reason"])
        ...
        self.assertEqual("media-image-1", approval_creator.submission.attachment_media_id)
        self.assertEqual("allow", body_fifth["permission_decision"])

    def test_reimbursement_recognition_confirm_accepts_confirm_request_alias(self) -> None:
        approval_creator = _StubReimbursementApprovalCreator(
            ReimbursementApprovalResult(success=True, reason="submitted", process_instance_id="proc-rmb-api-alias-1")
        )
        ...
        client.post(
            ...
            json=make_stream_payload(
                text="我要报销差旅费",
                sender_id="finance-user",
                conversation_id="conv-rmb-api-alias-1",
            ),
        )
        client.post(
            "/dingtalk/stream/events",
    ...
    def test_reimbursement_recognition_requires_confirmation_before_company_selection(self) -> None:
        ...
        self.assertEqual("reimbursement_travel_recognition_confirmation", blocked.json()["reason"])
        ...
        self.assertEqual(200, fourth.status_code)
        body_fourth = fourth.json()
        self.assertEqual("reimbursement_travel_collecting_company", body_fourth["reason"])
        self.assertEqual("reimbursement_recognition_confirm", body_fourth["reimbursement_action"])
        self.assertEqual("pending", body_fourth["reimbursement_status"])
        ...
        self.assertEqual("interactive_card", body_fifth["reply"]["channel"])
